...
// Create theme options
	...
	$options = array (
		...
		array("type" => "whole_width", "html" => "<hr/><br/><b>These 2 images will appear either side of the video</b>"),
 ...
 //---
		array( "name" => "Home Page Middle Video Embed",
			"desc" => "The play-in-place video. Enter just the video ID here",
			"id" => $shortname."_home_vid_id",
			"type" => "text",
			"std" => "",
			"class" => "regular-text"),
 ...
 //---
		...
		array( "name" => "Delivery tab content",
			"desc" => "The content in the Delivery/Return tab for products",
			"id" => $shortname."_general_tab",
			"type" => "textarea",
			"std" => "",
			"class" => "regular-text"),
		...
		<tr valign="top" class="options_input options_textarea">
			...
			<tr class="options_input options_select">
				...
				<?php foreach ($value['options'] as $key=>$option) {
					...
					$radio_setting = $sa_settings[$value['id']];
					if($radio_setting != ''){
					...
					<label>
					<input type="radio" name="sa_options[<?php echo $value['id']; ?>]" value="<?php echo $key; ?>" <?php echo $checked; ?> /> <span><?php echo $option; ?></span></label><br>
					<?php } ?>
				</td>
					<td><span class="description"><?php echo $value['desc']; ?></span></td>
				</tr>
